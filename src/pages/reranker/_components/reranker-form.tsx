...
export function RerankerForm({ initialData, onSuccess }: RerankerFormProps) {
  const [schemas, setSchemas] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    RerankerService.getSchemas().then((s) => {
      setSchemas(s);
      setLoading(false);
    });
  }, []);
  ...
  const {
    ...
    defaultValues: {
      ...
      provider: initialData?.provider || "COHERE",
    },
  });
  ...
  const onSubmit = async (baseValues: BaseValues, configValues: any) => {
    ...
    if (initialData) {
      await RerankerService.updateReranker(initialData.id, finalData as any);
    } else {
      await RerankerService.createReranker(finalData as any);
    }
    onSuccess();
  };
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input
          ...
          placeholder="e.g. Cohere Rerank Production"
          {...register("name")}
      ...
      {currentSchema && (
        <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
          <SchemaForm
            ...
            submitLabel={initialData ? "Update Re-ranker" : "Create Re-ranker"}
          />
        </div>
      )}
    </div>
